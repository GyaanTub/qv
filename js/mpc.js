...
async function calculateMonthlyPercentageChange(stockData) {
    ...
    stockData.forEach(item => {
        ...
        if (!percentageByMonth[yearMonthKey]) {
            percentageByMonth[yearMonthKey] = { totalChange: 0, count: 0 };
        }

        // Add the volatility to the corresponding year-month key
        percentageByMonth[yearMonthKey].totalChange += change;
        percentageByMonth[yearMonthKey].count += 1;
    });

    // Create an array to store the result in the desired format
    const averagePercentageArray = [];

    for (let key in percentageByMonth) {
        const { totalChange, count } = percentageByMonth[key];
        const [year, month] = key.split('-');
        const averageChange = totalChange;

        // Add the result as an object to the array with numbers for year and month
        averagePercentageArray.push({
...
async function createChart(stockName) {
    ...
    let volData = await calculateMonthlyPercentageChange(stockData);
    // console.log(volData);
